...
`, Timeout: 60},
	...
	"apply_passthrough_domain": {
		...
		Description: "Configure SSL passthrough (Layer 4) for a domain using nginx stream module",
		Category:    "domains",
		Variables: []VariableDef{
			{Name: "domain", Type: "string", Required: true, Description: "Domain name"},
...
if ! grep -q "^stream {" "$NGINX_CONF"; then
    ...
    # First, check if there's content after the http block
    if grep -q "^stream" "$NGINX_CONF"; then
        echo "Stream block already exists (maybe inline)"
    else
        # Add stream block at the end of the file
        echo "" >> "$NGINX_CONF"
        echo "# SSL Passthrough configuration" >> "$NGINX_CONF"
    ...
    if ! grep -q "include /etc/nginx/stream.d" "$NGINX_CONF"; then
        # Add include to existing stream block
        sed -i '/^stream {/a\    '"$STREAM_INCLUDE"'' "$NGINX_CONF"
        echo "Added stream.d include to existing stream block"
    fi
fi
`, Timeout: 30},
			// Write stream config for this domain
			{Action: "file", Op: "write", Path: "/etc/nginx/stream.d/{{domain}}.conf", Mode: "0644", Content: `# SSL Passthrough for {{domain}}
# This forwards raw TLS traffic to the backend without termination
# The backend server handles SSL certificates

upstream passthrough_{{domain | replace "." "_" | replace "-" "_"}} {
    server {{target}};
}

# Note: This config is included in the main stream {} block
# SNI routing is handled by the map in nginx.conf if multiple passthroughs exist
`},
			// Create/update the SNI map configuration
			{Action: "exec", Command: `
STREAM_MAP="/etc/nginx/stream.d/00-sni-map.conf"
DOMAIN="{{domain}}"
TARGET="{{target}}"
UPSTREAM_NAME="passthrough_$(echo "$DOMAIN" | tr '.-' '_')"

# Create or update SNI map file
if [ ! -f "$STREAM_MAP" ]; then
    cat > "$STREAM_MAP" << 'MAPEOF'
# SNI-based routing for SSL passthrough
...
server {
    ...
    # Only proxy if we have a matching backend
    # Otherwise, let the http block handle it (return empty = close connection for unknown domains)
...
# Add the new entry before the default line
sed -i "/default \"\";/i\\    $DOMAIN $TARGET;" "$STREAM_MAP"

echo "Updated SNI map for $DOMAIN -> $TARGET"
