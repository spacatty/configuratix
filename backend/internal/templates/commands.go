...
`},
			// Create/update the SNI map configuration with PROXY Protocol
			{Action: "exec", Command: `
STREAM_MAP="/etc/nginx/stream.d/00-sni-map.conf"
DOMAIN="{{domain}}"
TARGET="{{target}}"

# Create or update SNI map file with PROXY Protocol support
...
#
# IMPORTANT: Your backend nginx must be configured to accept PROXY Protocol:
#
#   server {
#       listen 443 ssl proxy_protocol;
#       
#       # Trust the proxy server to provide real IP
#       set_real_ip_from 0.0.0.0/0;  # Or specify proxy IP
...
server {
    ...
    proxy_protocol on;           # Send PROXY Protocol header to backend
...
echo "Updated SNI map for $DOMAIN -> $TARGET (with PROXY Protocol)"
